...
5. QA visual autenticado realizado em 360, 390, 768 e 1280 px, em claro/escuro. Campos numéricos e observações da avaliação escondem a navegação inferior quando recebem foco. `prefers-reduced-motion` foi conferido nas regras CSS existentes.
6. O smoke automatizado com Chromium cobre o registro do worker em contexto limpo, remoção de caches antigos, abertura da página e fallback offline. A auditoria encontrou e corrigiu a ausência do bootstrap do service worker na rota do template unificado. Continua pendente conferir uma PWA física já instalada no dispositivo do aluno após o deploy desta correção.
...
- A auditoria pós-deploy encontrou que `/renan/<slug>` não injetava manifest, prompt de instalação nem registro do service worker ao renderizar `workout.html`, porque esse shell não estende `_base.html`. O bootstrap agora é injetado pela fronteira de renderização, mantendo o CSS e a estrutura próprios do novo layout. O smoke E2E confirmou cache versionado, limpeza de caches antigos, precache de CSS/JS do shell e abertura offline da página e do fallback. A validação numa PWA física existente ainda depende de executar o smoke após o próximo deploy.
