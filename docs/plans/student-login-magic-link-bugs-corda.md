...
REVISAO 6 (2026-09-16 — teste ao vivo com Postgres real pela primeira vez, nao so leitura de codigo/pytest):
- ambiente ganhou Postgres 16 local + venv Python 3.12 + Django dev server real nesta sessao (antes disso, todo o plano so tinha sido validado por leitura de codigo e pytest com `--create-db`). Isso permitiu, pela primeira vez, testar de navegador de verdade (Playwright) em vez de so unit test.
- suite completa de `student_identity/ signup/ access/ tests/test_error_scenarios.py tests/test_student_identity_oauth_journeys.py tests/test_student_identity_membership_actions.py tests/test_tenant_boundary.py` rodada com `--create-db --migrations`: 251 passed, 6 skipped (gap pre-existente e sem relacao com login, em `test_tenant_boundary.py` — tabela `boxcore_student` ausente num cenario de schema, fora do escopo deste plano). As 4 falhas iniciais (`resend-network-error`) eram causadas pelo `.env` de teste desta sessao ter `STUDENT_EMAIL_PROVIDER=resend` (o default real e `smtp`) tentando rede real bloqueada pelo sandbox — nao sao bug do app; corrigido no `.env` local e as 4 passaram.
- Onda 4b (opcional, unificar `403.html` — nao tinha onda numerada antes, tratada como item solto do Bug 4) foi implementada e confirmada por screenshot real: layout, `{{ exception|default:... }}` mostrando a mensagem real do `RoleRequiredMixin`, tudo correto.
- Onda 5, item pendente (trocar `_flash_stack.html`/`checkout.html`/`onboarding.html` para `state_notice.html`) foi finalmente implementado E verificado visualmente (Playwright, nao so leitura de CSS): `states.css` adicionado aos 3 bundles (`_head_assets.html` do app do aluno, `<head>` de `checkout.html` e `onboarding.html`); screenshot confirma as 4 tonalidades (danger/warning/success/info) renderizando com cor e contraste corretos, tokens de design resolvendo bem. `.student-flash`/`.student-flash--*` (agora orfaos em `flash.css`) foram removidos — nenhum outro arquivo os referenciava (confirmado por grep antes de apagar). `.checkout-message`/`.checkout-message--*` NAO foram removidos de `signup-checkout.css`: `templates/signup/success.html` (fora do escopo deste plano) ainda os usa. Suite completa re-rodada apos a mudanca: 184 passed, 0 regressao.
- Bug 1 (erro ao gerar link do grupo) tentado ao vivo, varias vezes (criar link, criar de novo/regenerar 3x, pausar) com um Owner recem-criado numa Box de dev limpa: **nao reproduziu em nenhuma tentativa**. Continua sem causa raiz confirmada — a rede de seguranca da Onda 0 segue sendo a mitigacao ate aparecer um traceback real de producao. Hipotese nao testada: pode depender de um estado pre-existente (ex.: link ja revogado, superdev com Membership em varios boxes, ou uma corrida de concorrencia) que uma Box de dev recem-criada nao reproduz.
- achado novo, fora do escopo original deste plano mas descoberto ao investigar a arquitetura de login para a pergunta separada de "login em outra porta" (ver `docs/plans/student-login-service-isolation-corda.md`): `config/urls_public.py` e a setting `PUBLIC_SCHEMA_URLCONF` estao **mortos** — nenhum middleware (`django_tenants.middleware.TenantMainMiddleware` nao esta em `MIDDLEWARE`) jamais troca `request.urlconf` para eles. `ROOT_URLCONF` e sempre `config.urls`, para toda requisicao, tenant ou nao. O docstring de `urls_public.py` afirma o contrario ("django-tenants roteie requisicoes para este URLCONF quando o schema e public") — isso e falso na configuracao atual e pode enganar quem ler o arquivo depois. Nao mexido nesta revisao (fora do escopo dos bugs de login); registrado aqui para nao ser redescoberto do zero. A isolacao real de "essa rota nao precisa de tenant" e feita inteiramente por `control.middleware.TenantBySessionMiddleware.PUBLIC_SCHEMA_PATHS` (whitelist de prefixo de path, independente de dominio/porta) — nao por troca de urlconf.
-->
