...
- Validação automatizada repetida: 246 testes direcionados e `check_static_drift --strict` passaram. Um novo smoke E2E com Chromium passou para registro do worker num contexto limpo, descarte de caches antigos e uso offline do shell/estilos. A verificação numa PWA física existente permanece pendente após o deploy desta correção.
