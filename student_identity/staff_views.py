...
from .funnel_events import build_student_onboarding_event_action, record_student_onboarding_event
...
class StudentInvitationOperationsView(LoginRequiredMixin, RoleRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'create-box-link':
            return self._handle_create_box_link(request)
        if action == 'pause-box-link':
            return self._handle_pause_box_link(request)
    ...
    def get_context_data(self, **kwargs):
        ...
        journey_funnels = self._build_journey_funnels(
            box_root_slug=box_root_slug,
            active_box_invite_link=active_box_invite_link,
        )
        priority_of_day = self._build_priority_of_day(
        ...
        recommended_queue = self._build_recommended_queue(
            ...
            active_box_invite_link=(
                {
                    ...
                    'status_label': self._build_box_invite_link_status_label(active_box_invite_link),
                    'status_tone': self._build_box_invite_link_status_tone(active_box_invite_link),
                }
                if active_box_invite_link is not None else None
            ),
        )
        attach_page_payload(
            context,
    ...
    def _build_journey_funnels(self, *, box_root_slug: str, active_box_invite_link):
        ...
        journeys = [
            {
                ...
                'scope_queryset': (
                    ...
                    if active_box_invite_link is not None else
                    base_queryset
                ),
            },
            {
            ...
            funnels.append(
                {
                    ...
                    'headline_tone': 'ok' if conversion_rate >= 60 else 'attention' if conversion_rate >= 30 else 'danger' if first_step_value else 'muted',
                    ...
                    'summary_cards': [
                        ...
                        },
                        {
                            ...
                            'tone': 'ok' if conversion_rate >= 60 else 'attention' if conversion_rate >= 30 else 'danger' if first_step_value else 'muted',
                        },
                    ],
                    'alerts': alerts,
                }
    ...
    def _build_journey_next_action(self, *, journey: str, first_step_value: int, conversion_rate: float, biggest_drop):
        if first_step_value == 0:
            copy_by_journey = {
                ...
                StudentOnboardingJourney.IMPORTED_LEAD_INVITE: 'A recepcao pode começar pelos leads mais quentes usando o botao de 1 clique no WhatsApp.',
            ...
            return {
                ...
                'body': copy_by_journey.get(journey, 'Comece gerando volume neste corredor para o painel aprender.'),
                'tone': 'muted',
                'pill': 'Sem movimento',
            }
        ...
        journey_actions = {
            ...
            StudentOnboardingJourney.REGISTERED_STUDENT_INVITE: {
                ...
                ('Landing aberta', 'OAuth concluido'): 'A landing esta atraindo clique, mas ainda falta confiança para autenticar. Simplifique a explicacao e destaque que o acesso e rapido.',
    ...
    def _build_box_invite_link_status_label(self, link: StudentBoxInviteLink) -> str:
        if link.is_revoked:
            return 'Revogado'
        if link.is_paused:
            return 'Pausado'
    ...
    def _build_box_invite_link_status_tone(self, link: StudentBoxInviteLink) -> str:
        if link.can_accept:
            return 'ok'
        if link.is_paused:
            return 'attention'
