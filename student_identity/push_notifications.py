...
def send_session_cancelled_push(*, session, box_root_slug: str) -> int:
    """
    Deprecado como callsite direto. A entrega agora acontece via signal:

        operations.signals.session_cancellation.student_session_cancelled
        -> student_identity.listeners._on_session_cancelled

    Esta funcao e mantida apenas para compatibilidade com chamadas legadas
    e retorna 0 sem side effects quando chamada fora do fluxo de signal.
    O fluxo canonico e acionar o cancelamento via ClassSession.save() com
    status='canceled'; o signal e emitido automaticamente pelo post_save hook
    registrado em operations.apps.OperationsConfig.ready().
    """
    return 0
