...
def read_student_oauth_callback_input(*, request, provider: str) -> StudentOAuthCallbackInput:
    ...
    if error:
        raise StudentOAuthCallbackPolicyError('O provedor cancelou ou recusou a autenticação.')
    ...
    if state_payload is None or state_payload.get('provider') != provider:
        raise StudentOAuthCallbackPolicyError('O estado da autenticação não é válido. Tente novamente.')

    code = (request.GET.get('code') or request.POST.get('code') or '').strip()
    if not code:
        raise StudentOAuthCallbackPolicyError('O provedor não retornou um código de autenticação válido.')
