"""
ARQUIVO: testes de GET /renan/<slug>/meus-dados.json (Onda A3 — export de
dados do titular, docs/plans/public-workouts-produtizacao-corda.md).

POR QUE ELE EXISTE:
- mesma regra de auth dos demais endpoints de conta (pacote.json, carga),
  testada aqui pelo lado do export: sessao de login + gate de posse do
  slug antes de devolver qualquer dado pessoal.
"""
...
class PublicWorkoutExportDataEndpointTests(TestCase):
    def _url(self, slug='bruno'):
        return reverse('public-workout-export-data', kwargs={'plan_slug': slug})
    ...
    def test_owner_gets_full_export(self):
        account = _make_account_with_subscription(email='bruno@example.com', plan_slug='bruno')
        record_load(
            account_id=account.pk, movement_slug='agachamento-livre', weight_kg=Decimal('100'),
            performed_on='2026-01-05', idempotency_key='key-1',
        )
        _login(self.client, account.pk)
        ...
        body = response.json()
        self.assertEqual(body['account']['email'], 'bruno@example.com')
        self.assertEqual(body['subscription']['plan_slug'], 'bruno')
        self.assertEqual(len(body['load_history']), 1)
